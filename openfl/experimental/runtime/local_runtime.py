...
class LocalRuntime(Runtime):
    ...
    def execute_foreach_task(
        ...
        if self.backend == "ray":
            ...
            flspec_class._clones.update(
                {col: obj for col, obj in zip(selected_collaborators, clones)}
            )
            del ray_executor
            del clones
            gc.collect()
        ...
        func = clone.execute_task_args[0].__name__
...
def collaborator_step_executor(clone, to_exec):
    ...
    while not_at_transition_point:
        ...
        to_exec = getattr(clone, f.__name__)
        print("to execute", to_exec.__name__)
